...
public class WatiApiCall {
	...
	public CompletableFuture<String> sendMsg(OkHttpClient okhttp, RequestBody body, String commonurl, String msgurl,
		...
		try {
			...
			String isValidationApi = StringUtils.isBlank(detail.getIsvalidationapi()) ? "N"
					: detail.getIsvalidationapi();
			...
			if ("Y".equalsIgnoreCase(isResMsg)) {
				...
				if (StringUtils.isNotEmpty(isResMsgApi)) {
					...
					String mesText = detailRepo.getMessageText(waid, currentStageCode.toString(),
							subStageCode.toString());
				 ...
				 * isTemplateMsg=detailRepo.getTemplateStatus(waid,detail.getReqDetPk().
				 * getCurrentstage().toString()
				 * ,detail.getReqDetPk().getCurrentsubstage().toString(),detail.getRemarks());
				 */

			}

			WAWatiReq waReq = WAWatiReq.builder().filepath(detail.getFile_path()).msg(detail.getMessage()).waid(waid)
					.build();
			...
			if (fileyn.equalsIgnoreCase("Y")) {
				...
				if (StringUtils.isNotBlank(waReq.getMsg()) && !"R".equals(status)) {
					...
					String msgEn = StringUtils.isBlank(wamsgM.getMessage_content_en()) ? ""
							: wamsgM.getMessage_content_en();
					String msgAr = StringUtils.isBlank(wamsgM.getMessage_content_ar()) ? ""
							: wamsgM.getMessage_content_ar();
					String interactive_button_yn = StringUtils.isBlank(wamsgM.getInteractiveButtonYn()) ? "N"
							: wamsgM.getInteractiveButtonYn();
					String isCtaDynamicYn = StringUtils.isBlank(wamsgM.getIsCtaDynamicYn()) ? "N"
							: wamsgM.getIsCtaDynamicYn();
					String ctaButtonUrl = StringUtils.isBlank(wamsgM.getCtaButtonUrl()) ? "N"
							: wamsgM.getCtaButtonUrl();
					String ctaButtonkeys = StringUtils.isBlank(wamsgM.getCtaButtonKeys()) ? "N"
							: wamsgM.getCtaButtonKeys();
					...
					String msg = "D".equals(detail.getStatus()) ? detail.getMessage()
							: "English".equalsIgnoreCase(language) ? msgEn : msgAr;

					String button1 = "", button2 = "", button3 = "", flow_id = "", flow_token = "",
							flowRequestDataYn = "", flow_api = "", flow_api_auth = "", flow_api_method = "",
							flow_button_name = "", cta_button_name = "", location_button_name = "",
							menu_button_name = "", flow_index_screen_name = "", flow_api_request = "";
					...
					if ("Y".equalsIgnoreCase(interactive_button_yn)) {
						...
						if ("FLOW".equalsIgnoreCase(message_type)) {
							...
							flowRequestDataYn = StringUtils.isBlank(wamsgM.getRequestdataYn()) ? "N"
									: wamsgM.getRequestdataYn();
							...
							flow_api_method = StringUtils.isBlank(wamsgM.getFlowApiMethod()) ? ""
									: wamsgM.getFlowApiMethod();
							flow_id = StringUtils.isBlank(wamsgM.getFlowId()) ? "" : wamsgM.getFlowId();
							flow_index_screen_name = StringUtils.isBlank(wamsgM.getFlow_index_screen_name()) ? ""
									: wamsgM.getFlow_index_screen_name();
							flow_api_request = StringUtils.isBlank(wamsgM.getFlowApiRequest()) ? ""
									: wamsgM.getFlowApiRequest();
						...
						if ("English".equalsIgnoreCase(language)) {
							...
							flow_button_name = StringUtils.isBlank(wamsgM.getFlowButtonName()) ? ""
									: wamsgM.getFlowButtonName();
							cta_button_name = StringUtils.isBlank(wamsgM.getCtaButtonName()) ? ""
									: wamsgM.getCtaButtonName();
							location_button_name = StringUtils.isBlank(wamsgM.getLocButtonName()) ? ""
									: wamsgM.getLocButtonName();
							menu_button_name = StringUtils.isBlank(wamsgM.getMenu_button_name()) ? ""
									: wamsgM.getMenu_button_name();
							...
							flow_button_name = StringUtils.isBlank(wamsgM.getFlowButtonNameSw()) ? ""
									: wamsgM.getFlowButtonNameSw();
							cta_button_name = StringUtils.isBlank(wamsgM.getCtaButtonNameSw()) ? ""
									: wamsgM.getCtaButtonNameSw();
							location_button_name = StringUtils.isBlank(wamsgM.getLocButtonNameSw()) ? ""
									: wamsgM.getLocButtonNameSw();
							menu_button_name = StringUtils.isBlank(wamsgM.getMenu_button_name_sw()) ? ""
									: wamsgM.getMenu_button_name_sw();

						}
					}
					WAWatiReq waRequest = WAWatiReq.builder().filepath("").msg(msg).waid(String.valueOf(waid))
							.button_1(button1).button_2(button2).button_3(button3).messageId(wamsgM.getRemarks())
							.flow_button_name(flow_button_name).flowApi(flow_api).flowId(flow_id).flowToken(flow_token)
							.flowApiAuth(flow_api_auth).flowApiMethod(flow_api_method)
							.flow_requestdata_yn(flowRequestDataYn).cta_button_name(cta_button_name)
							.location_button_name(location_button_name).messageType(message_type)
							.menu_button_name(menu_button_name).flow_index_screen_name(flow_index_screen_name)
							.flow_api_request(flow_api_request).interactiveYn(interactive_button_yn)
							.isCtaDynamicYn(isCtaDynamicYn).ctaButtonUrl(ctaButtonUrl).ctaButtonKeys(ctaButtonkeys)
							.build();
		...
		try {
			...
			for (Map<String, Object> map : list) {
				...
				if (response.contains(key)) {
					...
					String emoji = EmojiParser
							.parseToUnicode(map.get("REMARKS") == null ? "" : map.get("REMARKS").toString().trim());
	...
	public String sendValidationMsg(OkHttpClient okhttp, RequestBody body, String commonurl, String msgurl,
		...
		try {
			...
			String isResMsg = StringUtils.isBlank(detail.getIsResMsg()) ? "N" : detail.getIsResMsg();
			// String isButtonMsg=
			// StringUtils.isBlank(tempM.getIsButtonMsg())?"N":tempM.getIsButtonMsg();
			// String msgType=
			// StringUtils.isBlank(tempM.getMsgType())?"":tempM.getMsgType();
			// String imageName =
			// StringUtils.isBlank(tempM.getImageName())?"":tempM.getImageName();
			// String imageUrl =
			// StringUtils.isBlank(tempM.getImageUrl())?"":tempM.getImageUrl();
			// String language =contactDataRepo.getLanguage(waid);
			// String button1 ="",button2="",button3="";
			/*
			 * if("Y".equalsIgnoreCase(isButtonMsg)) {
			 * if("English".equalsIgnoreCase(language)) { button1=tempM.getButton1();
			 * button2=tempM.getButton2();
			 * button3=StringUtils.isBlank(tempM.getButton3())?"":tempM.getButton3(); }else
			 * if("Swahili".equalsIgnoreCase(language)) { button1=tempM.getButtonSw1();
			 * button2=tempM.getButtonSw2();
			 * button3=StringUtils.isBlank(tempM.getButtonSw3())?"":tempM.getButtonSw3(); }
			 * } String isTemplateMsg
			 * =StringUtils.isBlank(detail.getIsTemplateMsg())?"N":detail.getIsTemplateMsg()
			 * ; String
			 * buttonUrl=cs.getwebserviceurlProperty().getProperty("whatsapp.api.button");
			...
			if ("Y".equalsIgnoreCase(isResSaveApi) && "Y".equalsIgnoreCase(apiCall)) {
				...
				if (StringUtils.isNotBlank(error_desc)) {
					String errorResStr = StringUtils.isBlank(tempM.getErrorrespstring()) ? ""
							: tempM.getErrorrespstring();
					for (Map.Entry<String, Object> entry : data.entrySet()) {
					...
					WAWatiReq waReq = WAWatiReq.builder().filepath(detail.getFile_path()).msg(errorResStr).waid(waid)
				...
				if (StringUtils.isNotBlank(apiResp)) {
					...
					/*
					 * WAWatiReq waReq = WAWatiReq.builder() .filepath(detail.getFile_path())
					 * .msg(apiResp) .waid(waid) //.isButtonMsg(isButtonMsg) //.msgType(msgType)
					 * //.imageUrl(imageUrl) //.imageName(imageName) //.button1(button1)
					 * //.button2(button2) //.button3(button3) //.isTemplateMsg(isTemplateMsg)
					 * .build();
					 */
				...
				if (StringUtils.isNotBlank(apiResp)) {
					...
					WAWatiReq waReq = WAWatiReq.builder().filepath(detail.getFile_path()).msg(apiResp).waid(waid)
							// .isButtonMsg(isButtonMsg)
							// .msgType(msgType)
							// .imageUrl(imageUrl)
							// .imageName(imageName)
	...
	public String sendDocValidationMsg(OkHttpClient okhttp, RequestBody body, String commonurl, String msgurl,
		...
		try {
			 ...
			 * StringUtils.isBlank(tempM.getIsButtonMsg())?"N":tempM.getIsButtonMsg();
			 * String msgType=
			 * StringUtils.isBlank(tempM.getMsgType())?"":tempM.getMsgType(); String
			 * imageName =
			 * StringUtils.isBlank(tempM.getImageName())?"":tempM.getImageName(); String
			 * imageUrl = StringUtils.isBlank(tempM.getImageUrl())?"":tempM.getImageUrl();
			 * String language =contactDataRepo.getLanguage(waid); String button1
			 * ="",button2="",button3=""; if("Y".equalsIgnoreCase(isButtonMsg)) {
			 * if("English".equalsIgnoreCase(language)) { button1=tempM.getButton1();
			 * button2=tempM.getButton2();
			 * button3=StringUtils.isBlank(tempM.getButton3())?"":tempM.getButton3(); }else
			 * if("Swahili".equalsIgnoreCase(language)) { button1=tempM.getButtonSw1();
			 ...
			 * buttonUrl=cs.getwebserviceurlProperty().getProperty("whatsapp.api.button");
			 * 
			 * String url ="Y".equalsIgnoreCase(isButtonMsg)? commonurl +
			 * buttonUrl:commonurl + msgurl;
			 */
		...
		try {
			...
			if (exist) {
				...
				RequestBody body = new MultipartBody.Builder().setType(MultipartBody.FORM)
						.addFormDataPart("file", fileName, RequestBody.create(file, MediaType.parse(mimeType))).build();
				...
				if (result.equalsIgnoreCase("success")) {
					...
					if (apiRes.getMessage() != null) {
						...
						MessageFileRes msg = objectMapper.convertValue(apiRes.getMessage(), MessageFileRes.class);
	...
	public String sendDocmentMsg(OkHttpClient okhttp, RequestBody body, String commonurl, String msgurl, String fileurl,
		...
		try {
			...
			if (StringUtils.isNotBlank(apiResp)) {
				...
				WAWatiReq waReq = WAWatiReq.builder().filepath(detail.getFile_path()).msg(apiResp).waid(waid).build();
	...
	public String sendIsResYnMsg(OkHttpClient okhttp, RequestBody body, String commonurl, String msgurl, String fileurl,
		...
		try {
			...
			if (StringUtils.isBlank(apiResp)) {
				...
				WAWatiReq waReq = WAWatiReq.builder().filepath(reqDet.getFile_path()).msg(reqDet.getMessage())
						.waid(waid).build();
				...
				WhatsappTemplateMaster temp = getTempMasterStageContent(reqDet.getRemarks(), "90016",
						Long.valueOf(waid), reqDet.getReqDetPk().getCurrentstage(),
						reqDet.getReqDetPk().getCurrentsubstage());
				 ...
				 * 
				 * ButtonMediaReq media =ButtonMediaReq.builder() .fileName(temp.getImageName())
				 * .url(temp.getImageUrl()) .build();
				 * 
				 * header =ButtonHeaderReq.builder().type(temp.getMsgType()).
				 * text(temp.getButtonHeader()) .media(media) .build(); }else {
				 * 
				 * header
				 * =ButtonHeaderReq.builder().type(StringUtils.isBlank(temp.getMsgType())?"Text"
				 * :temp.getMsgType()). text(temp.getButtonHeader()) .media(null) .build(); }
				 * 
				 * 
				 * List<ButtonsNameReq>buttons =new ArrayList<ButtonsNameReq>();
				 * 
				 * buttons.add(new
				 * ButtonsNameReq(StringUtils.isBlank(temp.getButton1())?"MainMenu":temp.
				 * getButton1())); if(StringUtils.isNotBlank(temp.getButton2())) {
				 * buttons.add(new ButtonsNameReq(temp.getButton2())); }
				 * if(StringUtils.isNotBlank(temp.getButton3())) { buttons.add(new
				 * ButtonsNameReq(temp.getButton3())); } WhatsAppButtonReq req =
				 * WhatsAppButtonReq.builder() .header(header) .body(apiResp)
				 * .footer(StringUtils.isBlank(temp.getButtonFooter())?"":temp.getButtonFooter()
				 * ) .buttons(buttons) .build();
				 * 
				 * RequestBody requestBody =RequestBody.create(cs.reqPrint(req),
				 * MediaType.parse("application/json"));
				 * 
				 * Request request = new Request.Builder() .url(url) .addHeader("Authorization",
				 * auth) .post(requestBody) .build();
				 * 
				 * Response response = okhttp.newCall(request).execute();
				 * 
				 * String responseString = response.body().string();
	...
	private WAWatiReq sendButtonMsg(WAWatiReq waReq, WhatsappTemplateMaster temp) {
		...
		try {
			...
			/*
			 * if("Y".equals(isApiCall) || "N".equals(Isreplyyn)) messageBody
			 * =StringUtils.isBlank(waReq.getMsg())?"Something went wrong":waReq.getMsg();
			 * else messageBody
			 * =StringUtils.isBlank(temp.getButtonBody())?"Something went wrong":temp.
			 * getButtonBody();
			 * 
			 * if("Image".equalsIgnoreCase(temp.getMsgType())) {
			 * 
			 ...
			 * 
			 * header =ButtonHeaderReq.builder().type(temp.getMsgType()).
			 * text(temp.getButtonHeader()) .media(media) .build();
			 * 
			 * }else if("Text".equalsIgnoreCase(temp.getMsgType())) {
			 * 
			 * header =ButtonHeaderReq.builder().type(temp.getMsgType()).
			 * text(temp.getButtonHeader()) .media(null) .build(); }
			 * 
			 * OkHttpClient okhttp = new OkHttpClient.Builder() .readTimeout(30,
			 ...
			 * buttons.add(new ButtonsNameReq(temp.getButton2())); }
			 * if(StringUtils.isNotBlank(temp.getButton3())) { buttons.add(new
			 * ButtonsNameReq(temp.getButton3())); }
			 * 
			 * WhatsAppButtonReq req = WhatsAppButtonReq.builder() .header(header)
			 * .body(messageBody)
			 * .footer(StringUtils.isBlank(temp.getButtonFooter())?"":temp.getButtonFooter()
			 * ) .buttons(buttons) .build();
			 * 
			 ...
			 * 
			 * String
			 * twoSlash=cs.getwebserviceurlProperty().getProperty("wa.hit.slash.two");
			 * String
			 * oneSlash=cs.getwebserviceurlProperty().getProperty("wa.hit.slash.one");
			 * 
			 * String msgReq =obj.replace(twoSlash, oneSlash);
			 * 
			 * System.out.println(msgReq);
			 * 
			 * RequestBody requestBody =RequestBody.create(msgReq,
			 * MediaType.parse("application/json"));
			 * 
			 * Request request = new Request.Builder() .url(url) .addHeader("Authorization",
			 * auth) .post(requestBody) .build();
			 ...
			 * log.info("Whatsapp button api response" +responseString);
			 * 
			 * log.info("callSendSessionMsg--> waid: " + waReq.getWaid() + " response: " +
			 * responseString);
			 * 
			 * SendSessionMsg apiRes = objectMapper.readValue(responseString,
			 * SendSessionMsg.class);
			 * 
			 * String result = apiRes.getResult();
			 * 
			 * String msgid = "", sessionid = "";
			 ...
			 * if (apiRes.getMessage()!=null) {
			 * 
			 * if (apiRes.getMessage() != null) { MessageSendRes msg =
			 * objectMapper.convertValue(apiRes.getMessage(), MessageSendRes.class);
			 * 
			 * cs.reqPrint(msg);
			 * 
			 * msgid = msg.getId();
			 ...
			 * sessionid = msg.getTicketId(); }
			 * 
			 * detail.setIssent("Y"); detail.setWamsgId(msgid);
			 * detail.setWaresponse(result); detail.setSessionid(sessionid);
			 * 
			 * } else { String msgRes = apiRes.getMessage() == null ? "" :
			 * String.valueOf(apiRes.getMessage()); String info =
			 * StringUtils.isBlank(apiRes.getInfo()) ? "" : apiRes.getInfo();
			 * 
			 * detail.setIssent("N"); detail.setWamsgId(msgid);
			 * detail.setWaresponse(StringUtils.isBlank(info) ? msgRes : info);
			 * detail.setSessionid(sessionid); }
			 */
	...
	public WhatsappTemplateMaster getTempMasterStageContent(String msgid, String agencycode, Long waid, Long stageCode,
			Long subStageCode) {
		try {
			...
			WhatsappTemplateMaster tempM = jpa.selectFrom(qtempM)
					.where(qtempM.remarks.eq(msgid).and(qtempM.status.equalsIgnoreCase("Y"))
							.and(qtempMPk.agencycode.eq(agencycode)).and(qtempM.ischatyn.equalsIgnoreCase("Y"))
							.and(qtempMPk.stagecode.eq(stageCode)).and(qtempMPk.stagesubcode.eq(subStageCode)))
					.orderBy(qtempM.stage_order.asc()).fetchFirst();
	...
	private String frameWhatsappMetaMsg(WAWatiReq req, OkHttpClient okhttp, RequestBody requestBody) {
		...
		try {
			...
			if ("Menu".equalsIgnoreCase(req.getMessageType())) {
				...
				List<WhatsappMessageMenuMaster> menuList = wmmmRpo
						.findByMessageIdAndStatusIgnoreCaseOrderByDisplayOrder(req.getMessageId(), "Y");
				...
				if ("Y".equals(req.getFlow_requestdata_yn())) {
					...
					if ("GET".equalsIgnoreCase(req.getFlowApiMethod())) {
						...
						Request request = new Request.Builder().url(url.trim())
								.addHeader("Authorization", req.getFlowApiAuth().trim()).get().build();
						...
						String input_req = StringUtils.isBlank(req.getFlow_api_request()) ? null
								: req.getFlow_api_request();

						requestBody = RequestBody.create(input_req, mediaType);

						Request request = new Request.Builder().url(req.getFlowApi().trim())
								.addHeader("Authorization", req.getFlowApiAuth().trim()).post(requestBody).build();
				...
				if ("Y".equals(req.getIsCtaDynamicYn())) {
					...
					for (Map.Entry<String, Object> entry : cta_data.entrySet()) {
						...
						if (responseStr.contains("{" + entry.getKey() + "}")) {
							...
							responseStr = responseStr.replace("{" + entry.getKey() + "}",
									entry.getValue() == null ? "" : entry.getValue().toString());
