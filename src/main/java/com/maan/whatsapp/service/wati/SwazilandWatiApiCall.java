...
@Service
public class SwazilandWatiApiCall {
		...
		try {
			...
			if (fileyn.equalsIgnoreCase("Y")) {
				...
				if(StringUtils.isNotBlank(waReq.getMsg()) && !"R".equals(status)) {
					...
					url=cs.getwebserviceurlProperty().getProperty("swaziland.message.api");
					
					auth=cs.getwebserviceurlProperty().getProperty("swaziland.message.auth");
	...
	private String frameWhatsappMetaMsg(WAWatiReq req, OkHttpClient okhttp, RequestBody requestBody) {
		...
		try {
			...
			if("Menu".equalsIgnoreCase(req.getMessageType())) {
				...
				section_rows.put("title", "Phoenix Swaziland Products");
